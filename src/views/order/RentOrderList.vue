  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="操作" width="180px">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status === 1 && scope.row.handStatus === -1" type="primary" size="small" @click="editData(scope.row)">发货</el-button>
          <el-button v-else-if="scope.row.status === 1 && scope.row.handStatus === 0" type="text" size="small" disabled="true">发货中</el-button>
          <el-button v-else-if="scope.row.status === 3 && scope.row.handStatus === 1" type="text" size="small" disabled="true">完成发货</el-button>
          <el-button v-else type="text" size="small" disabled="true">已失效</el-button>
        </template>
      </el-table-column>
    </el-table>
