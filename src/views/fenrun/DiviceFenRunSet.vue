...
<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="listQuery.deviceId" placeholder="请输入设备ID" style="width: 200px;" class="filter-item"/>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="设备ID">
        <template slot-scope="scope">
          <span>{{ scope.row.device.deviceId }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="false" align="center" label="设备类型">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    handleEdit(item) {
      // 获取设备城市省id
      this.cityId = parseInt(item.device.cityCode.id / 10000) * 10000
      // 获取地方慈善
